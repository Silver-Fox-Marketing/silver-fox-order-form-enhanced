        ...
        try:
            ...
            if config['filtering_rules']:
                try:
                    ...
                    logger.error(f"Invalid JSON in filtering_rules for {dealership_name}: {je}")
                    raise ValueError(f"Invalid filtering_rules JSON configuration for {dealership_name}")
            
            if config['output_rules']:
                try:
                    output_rules = json.loads(config['output_rules'])
                except json.JSONDecodeError as je:
                    logger.error(f"Invalid JSON in output_rules for {dealership_name}: {je}")
                    raise ValueError(f"Invalid output_rules JSON configuration for {dealership_name}")
